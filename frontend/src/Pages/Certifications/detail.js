...
function CertificationDetail() {
  ...
  if (!cert) return <Layout><div>Loading...</div></Layout>;
    ...
    <Layout>
      <section style={{
        ...
        <div style={{
          ...
          />
          <div style={{ color: "#a88c7d", fontSize: "18px", marginBottom: "12px" }}>
            Level: <b>{cert.level}</b> &nbsp;|&nbsp; Hours: <b>{cert.estimatedHours}</b>
          </div>
          <p style={{ color: "#444", marginBottom: "18px" }}>{cert.description}</p>
          <div style={{ textAlign: "left", marginBottom: "18px" }}>
            <strong style={{ color: "#ff6804" }}>Requirements:</strong>
            <ul>
              {cert.requirements.map((r, idx) => <li key={idx}>{r}</li>)}
            </ul>
          </div>

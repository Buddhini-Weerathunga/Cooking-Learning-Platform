                ...
                <div key={cert.id} style={cardStyle}>
                  <div style={{ display: "flex", alignItems: "center", width: "100%" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        <Link to={`/certifications/${cert.id}`} style={{ color: "#ff6804", textDecoration: "none" }}>
                          {cert.title}
                        </Link>
                      </h2>
                      <div style={{ color: "#a88c7d", fontSize: "18px", margin: "8px 0" }}>
                        Level: <b>{cert.level}</b> &nbsp;|&nbsp; Hours: <b>{cert.estimatedHours}</b>
                      </div>
                      <p style={{ color: "#444", margin: 0 }}>{cert.description}</p>
                    </div>
                  </div>
                  <div style={{ marginTop: "18px" }}>
                    <Link to={`/certifications/edit/${cert.id}`}>
                      <button style={{ ...btnStyle, background: "#ffd7b3", color: "#ff6804" }}>Edit</button>
                    </Link>
                    <button
